...
| **03** | [`03-cloudflare-r2-remote-cleanup.md`](03-cloudflare-r2-remote-cleanup.md) | `Complete` | Prune legacy pre-Monday SQL dumps in `r2:database/` and legacy root `uploads/` directory on Cloudflare R2 |
| **04** | [`04-pre-upload-checksum-and-script-optimizations.md`](04-pre-upload-checksum-and-script-optimizations.md) | `Complete` | Pre-upload checksum check & Cloudflare R2 server-side copy in `scripts/lib/common.sh`, conditional pull caching |
| **05** | [`05-re-export-testing-and-push-benchmark.md`](05-re-export-testing-and-push-benchmark.md) | `Complete` | Full Studio re-export, Playwright test suite validation, `./scripts/push.sh` and `./scripts/pull.sh` speed benchmarks |
