...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', {name: 'Pause partner logo scrolling'}).click();
    await page.evaluate(() => document.activeElement.blur());
    await page.mouse.move(0, 0);
    const paused = await left();
    await page.waitForTimeout(350);
    assert.equal(await left(), paused, 'Explicit pause persists outside the carousel');
    await page.getByRole('button', {name: 'Play partner logo scrolling'}).click();
    await page.evaluate(() => document.activeElement.blur());
    await page.mouse.move(0, 0);
    await page.waitForFunction(({selector, paused}) => document.querySelector(selector).scrollLeft > paused + 3, {selector: trackSelector, paused}, {timeout: 5000});
    assert.ok(await left() > paused + 3, 'Play resumes automatic movement');

    await page.emulateMedia({reducedMotion: 'reduce'});
    const reduced = await left();
    await page.waitForTimeout(350);
    assert.equal(await left(), reduced, 'Reduced motion stops autoplay');
    assert.equal(await page.locator('.lakehub-partners-toggle').isVisible(), false);
    await track.focus();
    await page.keyboard.press('ArrowRight');
    ...
    assert.equal(await short.locator('.lakehub-partners-toggle').isVisible(), false);
